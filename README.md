...
pnpm test                      # robots.txt 解析器单元测试（12 项）
...
同长度 Allow 优先、同名 token 组合并。**12 项单元测试覆盖**（`pnpm test`）。

爬虫名册共 22 个，按 `search` / `training` / `user` 三类标注，
并明确标注哪些会真正影响 AI 答案可见性。
