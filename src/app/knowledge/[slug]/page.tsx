...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: article.title, description: article.body.slice(0, 160), alternates: { canonical: article.url },
    openGraph: { type: "article", title: article.title, description: article.body.slice(0, 160), url: article.url, publishedTime: article.publishedAt, authors: [article.author] } };
}
...
export default async function KnowledgeArticle({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl px-5 py-10">
    ...
    <article className="mt-7">
      ...
      <div className="my-8 space-y-5 leading-8">{article.body.split(/\n\s*\n/).map((block, index) => {
        const heading = /^(#{1,6})\s+([^\n]+)$/.exec(block.trim());
        return heading ? <h2 key={index} className="pt-3 text-xl font-semibold">{heading[2]}</h2> : <p key={index} className="whitespace-pre-wrap break-words">{block}</p>;
      })}</div>
